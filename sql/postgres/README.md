...
| **`supabase_brin_impression_id.sql`** | Standalone `CREATE INDEX` for BRIN on `impression_id` (same as embedded in `benchmark_schema.sql`) when I already have the table and only need this index on an existing DB. |
